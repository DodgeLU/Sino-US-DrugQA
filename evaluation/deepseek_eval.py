...
def call_deepseek(prompt: str, api_key: str, model: str, base_url: str, temperature: float, max_tokens: int) -> str:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="DeepSeek evaluation (single model).")
    ...
    parser.add_argument("--output", default=os.path.join("..", "results", "deepseek.jsonl"), help="Output JSONL")
    ...
    parser.add_argument("--base-url", default=os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com"), help="API base URL")
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        for idx, (item, few_shot) in enumerate(workload, start=1):
            ...
            raw = call_deepseek(prompt, api_key, args.model, args.base_url, args.temperature, args.max_tokens)
