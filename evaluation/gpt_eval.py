...
def call_openai(prompt: str, api_key: str, model: str, base_url: str, temperature: float, max_tokens: int) -> str:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OpenAI evaluation (single model).")
    ...
    parser.add_argument("--output", default=os.path.join("..", "results", "gpt.jsonl"), help="Output JSONL")
    parser.add_argument("--seed", type=int, default=42, help="Random seed (reserved)")
    parser.add_argument("--model", default=os.getenv("OPENAI_MODEL", "GPT-5.2"), help="Model name")
    parser.add_argument("--base-url", default=os.getenv("OPENAI_BASE_URL", "https://api.openai.com"), help="API base URL")
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        for idx, (item, few_shot) in enumerate(workload, start=1):
            ...
            raw = call_openai(prompt, api_key, args.model, args.base_url, args.temperature, args.max_tokens)
